...
export default function BlogPostClient({
  ...
  const useSanity = !!sanityArticle;

  const article = useSanity ? sanityArticle : hardcoded;
  ...
  if (useSanity && sanityRelated && sanityRelated.length > 0) {
    related = sanityRelated;
  } else {
    const sameTag = hardcodedArticles.filter(
      (a) => a.slug !== slug && a.tag === article.tag
    );
    const others = hardcodedArticles.filter(
      (a) => a.slug !== slug && a.tag !== article.tag
    );
    ...
    <>
      ...
      <section className="py-12 lg:py-20 bg-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          {useSanity && article.body ? (
            <SanityBlockContent value={article.body} />
          ) : !useSanity && "body" in article && Array.isArray(article.body) ? (
            article.body.map(
              (block: { heading: string; text: string }, i: number) => (
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
          ...
          <FadeIn delay={0.1}>
            <p className="text-primary/60 text-[15px] leading-relaxed max-w-2xl mx-auto mb-8">
              Nos experts se déplacent gratuitement pour évaluer votre projet et
