...
export default function Header({ forceVisible = false }: { forceVisible?: boolean }) {
  ...
  return (
    <>
              ...
              if (link.dropdown) {
                return (
                  <li key={link.label} className="relative group">
                    <Tag
                      href={link.href}
                      className="font-medium text-[12px] xl:text-[14px] text-primary/70 hover:text-primary transition-colors relative after:absolute after:bottom-[-4px] after:left-0 after:h-[2px] after:w-0 hover:after:w-full after:bg-accent after:transition-all whitespace-nowrap flex items-center gap-1"
                    >
                      {link.label}
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                      <div className="bg-white rounded-[16px] shadow-xl shadow-black/10 p-2 min-w-[220px]">
      ...
      {scrolled && (
        <div
          className={`fixed top-20 left-0 right-0 z-50 px-4 sm:px-8 lg:hidden overflow-hidden transition-all duration-300 ${
                ...
                return (
                  <div key={link.label}>
                    <button
                      ...
                      className="w-full flex items-center justify-between px-4 py-3 text-primary/80 hover:text-primary hover:bg-secondary rounded-xl font-medium text-sm transition-colors"
                    >
                      {link.label}
