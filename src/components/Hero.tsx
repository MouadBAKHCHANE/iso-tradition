        ...
        <div className="absolute top-0 left-0 right-0 z-20 px-6 sm:px-10 lg:px-14">
          ...
          <div className="flex items-start justify-between">
            ...
            <Link href="/" className="relative shrink-0 pt-6">
              <Image
                ...
                width={300}
                height={90}
                className="h-16 lg:h-16 xl:h-20 2xl:h-22 3xl:h-28 w-auto"
                priority
              />
            </Link>
            ...
            <div className="hidden lg:block relative">
              {/* White nav bar — centered, flush to top with concave notch ears */}
              <div className="bg-white rounded-b-[20px] px-5 lg:px-6 xl:px-10 2xl:px-12 3xl:px-16 py-3 lg:py-3 xl:py-4 2xl:py-5 3xl:py-7 flex items-center gap-4 lg:gap-5 xl:gap-8 2xl:gap-10 3xl:gap-14 relative">
                {navLinks.map((link) => {
                  ...
                  if (link.dropdown) {
                    return (
                      <div key={link.label} className="relative group">
                        <Tag
                          href={link.href}
                          className="font-medium text-[12px] lg:text-[12px] xl:text-[14px] 2xl:text-[15px] 3xl:text-[16px] text-primary/70 hover:text-primary transition-colors relative after:absolute after:bottom-[-3px] after:left-0 after:h-[2px] after:w-0 hover:after:w-full after:bg-accent after:transition-all whitespace-nowrap flex items-center gap-1"
                        >
                          {link.label}
                          <svg className="w-3 h-3 transition-transform duration-200 group-hover:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                          ...
                          <div className="bg-white rounded-[16px] shadow-2xl shadow-black/20 p-2 min-w-[240px] border border-gray-100">
                            {solutionsDropdown.map((item, i) => (
                              <Link
                                key={item.href}
                                href={item.href}
                                className={`block px-4 py-2.5 rounded-xl text-[13px] font-medium text-primary/70 hover:text-primary hover:bg-secondary transition-colors ${i === 0 ? "font-bold text-primary border-b border-gray-100 mb-1" : ""}`}
                              >
                                {item.label}
                              </Link>
                            ))}
                  ...
                  return (
                    <Tag
                      key={link.label}
                      href={link.href}
                      className="font-medium text-[12px] lg:text-[12px] xl:text-[14px] 2xl:text-[15px] 3xl:text-[22px] text-primary/70 hover:text-primary transition-colors relative after:absolute after:bottom-[-3px] after:left-0 after:h-[2px] after:w-0 hover:after:w-full after:bg-accent after:transition-all whitespace-nowrap"
                    >
                      {link.label}
                    </Tag>
                  );
          ...
          <AnimatePresence>
            {mobileOpen && (
              ...
              >
                <div className="bg-white rounded-[20px] shadow-lg px-6 py-5 space-y-1 max-h-[75vh] overflow-y-auto">
                  {navLinks.map((link) => {
                    const isInternal = link.href.startsWith("/");
                    const Tag = isInternal ? Link : "a";
                    if (link.dropdown) {
                      return (
                        <div key={link.label}>
                          <button
                            ...
                            className="w-full flex items-center justify-between px-4 py-3 text-primary/80 hover:text-primary hover:bg-secondary rounded-xl font-medium text-[15px] transition-colors"
                          >
                            {link.label}
                            ...
                            <div className="pl-4 pb-1 space-y-0.5">
                              {solutionsDropdown.map((item, i) => (
                                <Link
                                  ...
                                  className={`block px-4 py-2.5 rounded-xl text-[13px] transition-colors ${i === 0 ? "font-bold text-primary hover:bg-secondary" : "text-primary/70 hover:text-primary hover:bg-secondary"}`}
                                >
                                  {item.label}
                                </Link>
                              ))}
        ...
        <div className="relative z-10 flex flex-col justify-end min-h-[85vh] sm:min-h-screen lg:min-h-[85vh] 2xl:min-h-[84vh] 3xl:min-h-[82vh] px-5 sm:px-12 lg:px-20 2xl:px-24 3xl:px-28 pb-8 sm:pb-12 lg:pb-16 2xl:pb-18 3xl:pb-20">
          <div className="max-w-xl xl:max-w-3xl 2xl:max-w-3xl 3xl:max-w-4xl">
            ...
            >
              ...
              >
                <span className="flex items-center justify-center w-10 h-10 2xl:w-12 2xl:h-12 3xl:w-16 3xl:h-16 rounded-full border border-white/25">
                  <svg className="w-4 h-4 2xl:w-5 2xl:h-5 3xl:w-8 3xl:h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                  </svg>
                </span>
                <span className="font-semibold text-sm 2xl:text-base 3xl:text-2xl">021 624 53 00</span>
              </a>
            </motion.div>
          </div>
